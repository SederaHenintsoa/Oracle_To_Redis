...
namespace OracleToRedisImport.Services
{
    class VoyageService
    {
        ...
        public VoyageService(OracleService ORS, JsonToRedisService RS)
        {
            oracle = ORS;
            redis = RS;
        }

        public MigrationResult MigrationVoyageToRedis()
        {
            ...
            {
                Console.WriteLine("Aucune donnée récupérée d'Oracle");
            ...
            foreach (var row in rows)
            {
                try
                {
                    ...
                    redis.SaveJsons("voyage:" + v.id_voyage, v);
                    voyages.Add(v);
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Erreur migration Voyage ligne : " + string.Join(", ", row.Select(kv => kv.Key + "=" + kv.Value))
                                      + "\nException : " + ex.Message);
                }
            }

            Console.WriteLine("Migration de " + voyages.Count + " Voyage terminée !");
           // return JsonConvert.SerializeObject(voyages, Formatting.Indented);
            return new MigrationResult
            {
                Table = "VOYAGES",
                Count = voyages.Count,
                Json = JsonConvert.SerializeObject(voyages, Formatting.Indented)
            };
        }
    }
}